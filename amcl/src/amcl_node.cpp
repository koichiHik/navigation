...
void CheckParamAvailability(std::string param_name, bool result) {
  if (!result) {
    ROS_ERROR("[AmclNode] Param read failure. : %s", param_name.c_str());
  }
}
} // namespace
} // namespace amcl
...
private:
  ...
  void standardDeviationDiagnostics(
...
AmclNode::AmclNode()
    : sent_first_transform_(false), latest_tf_valid_(false), map_(NULL),
      pf_(NULL), resample_count_(0), odom_(NULL), laser_(NULL),
      private_nh_("~"), initial_pose_hyp_(NULL), first_map_received_(false),
      first_reconfigure_call_(true) {
  boost::recursive_mutex::scoped_lock l(configuration_mutex_);

  std::string param_namespace = private_nh_.getNamespace();

  // Grab params off the param server
  CheckParamAvailability(
      param_namespace + "/" + "use_map_topic",
      private_nh_.param("use_map_topic", use_map_topic_, false));
  CheckParamAvailability(
      param_namespace + "/" + "first_map_only",
      private_nh_.param("first_map_only", first_map_only_, false));

  {
    double tmp;
    CheckParamAvailability(param_namespace + "/" + "gui_publish_rate",
                           private_nh_.param("gui_publish_rate", tmp, -1.0));
    gui_publish_period = ros::Duration(1.0 / tmp);
    CheckParamAvailability(param_namespace + "/" + "save_pose_rate",
                           private_nh_.param("save_pose_rate", tmp, 0.5));
    save_pose_period = ros::Duration(1.0 / tmp);
  }

  CheckParamAvailability(
      param_namespace + "/" + "laser_min_range",
      private_nh_.param("laser_min_range", laser_min_range_, -1.0));
  CheckParamAvailability(
      param_namespace + "/" + "laser_max_range",
      private_nh_.param("laser_max_range", laser_max_range_, -1.0));
  CheckParamAvailability(param_namespace + "/" + "laser_max_beams",
                         private_nh_.param("laser_max_beams", max_beams_, 30));
  CheckParamAvailability(
      param_namespace + "/" + "min_particles",
      private_nh_.param("min_particles", min_particles_, 100));
  CheckParamAvailability(
      param_namespace + "/" + "max_particles",
      private_nh_.param("max_particles", max_particles_, 5000));
  CheckParamAvailability(param_namespace + "/" + "kld_err",
                         private_nh_.param("kld_err", pf_err_, 0.01));
  CheckParamAvailability(param_namespace + "/" + "kld_z",
                         private_nh_.param("kld_z", pf_z_, 0.99));

  // Odometry
  CheckParamAvailability(param_namespace + "/" + "odom_alpha1",
                         private_nh_.param("odom_alpha1", alpha1_, 0.2));
  CheckParamAvailability(param_namespace + "/" + "odom_alpha2",
                         private_nh_.param("odom_alpha2", alpha2_, 0.2));
  CheckParamAvailability(param_namespace + "/" + "odom_alpha3",
                         private_nh_.param("odom_alpha3", alpha3_, 0.2));
  CheckParamAvailability(param_namespace + "/" + "odom_alpha4",
                         private_nh_.param("odom_alpha4", alpha4_, 0.2));
  CheckParamAvailability(param_namespace + "/" + "odom_alpha5",
                         private_nh_.param("odom_alpha5", alpha5_, 0.2));

  CheckParamAvailability(param_namespace + "/" + "do_beamskip",
                         private_nh_.param("do_beamskip", do_beamskip_, false));
  CheckParamAvailability(
      param_namespace + "/" + "beam_skip_distance",
      private_nh_.param("beam_skip_distance", beam_skip_distance_, 0.5));
  CheckParamAvailability(
      param_namespace + "/" + "beam_skip_threshold",
      private_nh_.param("beam_skip_threshold", beam_skip_threshold_, 0.3));
  CheckParamAvailability(param_namespace + "/" + "beam_skip_error_threshold_",
                         private_nh_.param("beam_skip_error_threshold_",
                                           beam_skip_error_threshold_, 0.9));

  CheckParamAvailability(param_namespace + "/" + "laser_z_hit",
                         private_nh_.param("laser_z_hit", z_hit_, 0.95));
  CheckParamAvailability(param_namespace + "/" + "laser_z_short",
                         private_nh_.param("laser_z_short", z_short_, 0.1));
  CheckParamAvailability(param_namespace + "/" + "laser_z_max",
                         private_nh_.param("laser_z_max", z_max_, 0.05));
  CheckParamAvailability(param_namespace + "/" + "laser_z_rand",
                         private_nh_.param("laser_z_rand", z_rand_, 0.05));
  CheckParamAvailability(param_namespace + "/" + "laser_sigma_hit",
                         private_nh_.param("laser_sigma_hit", sigma_hit_, 0.2));
  CheckParamAvailability(
      param_namespace + "/" + "laser_lambda_short",
      private_nh_.param("laser_lambda_short", lambda_short_, 0.1));
  CheckParamAvailability(param_namespace + "/" + "laser_likelihood_max_dist",
                         private_nh_.param("laser_likelihood_max_dist",
                                           laser_likelihood_max_dist_, 2.0));

  std::string tmp_model_type;
  CheckParamAvailability(param_namespace + "/" + "laser_model_type",
                         private_nh_.param("laser_model_type", tmp_model_type,
                                           std::string("likelihood_field")));
  ...
  CheckParamAvailability(param_namespace + "/" + "odom_model_type",
                         private_nh_.param("odom_model_type", tmp_model_type,
                                           std::string("diff")));
  ...
  CheckParamAvailability(param_namespace + "/" + "update_min_d",
                         private_nh_.param("update_min_d", d_thresh_, 0.2));
  CheckParamAvailability(
      param_namespace + "/" + "update_min_a",
      private_nh_.param("update_min_a", a_thresh_, M_PI / 6.0));
  CheckParamAvailability(
      param_namespace + "/" + "odom_frame_id",
      private_nh_.param("odom_frame_id", odom_frame_id_, std::string("odom")));
  CheckParamAvailability(param_namespace + "/" + "base_frame_id",
                         private_nh_.param("base_frame_id", base_frame_id_,
                                           std::string("base_link")));
  CheckParamAvailability(param_namespace + "/" + "global_frame_id",
                         private_nh_.param("global_frame_id", global_frame_id_,
                                           std::string("map")));
  CheckParamAvailability(
      param_namespace + "/" + "resample_interval",
      private_nh_.param("resample_interval", resample_interval_, 2));

  double tmp_tol;
  CheckParamAvailability(
      param_namespace + "/" + "transform_tolerance",
      private_nh_.param("transform_tolerance", tmp_tol, 0.1));
  CheckParamAvailability(
      param_namespace + "/" + "recovery_alpha_slow",
      private_nh_.param("recovery_alpha_slow", alpha_slow_, 0.001));
  CheckParamAvailability(
      param_namespace + "/" + "recovery_alpha_fast",
      private_nh_.param("recovery_alpha_fast", alpha_fast_, 0.1));
  CheckParamAvailability(
      param_namespace + "/" + "tf_broadcast",
      private_nh_.param("tf_broadcast", tf_broadcast_, true));

  // For diagnostics
  CheckParamAvailability(
      param_namespace + "/" + "std_warn_level_x",
      private_nh_.param("std_warn_level_x", std_warn_level_x_, 0.2));
  CheckParamAvailability(
      param_namespace + "/" + "std_warn_level_y",
      private_nh_.param("std_warn_level_y", std_warn_level_y_, 0.2));
  CheckParamAvailability(
      param_namespace + "/" + "std_warn_level_yaw",
...

  {
    ...
    CheckParamAvailability(
        param_namespace + "/" + "bag_scan_period",
  ...
  CheckParamAvailability(param_namespace + "/" + "restore_defaults",
                         private_nh_.hasParam("restore_defaults"));
  ...
  pose_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>(
      "amcl_pose", 2, true);
  particlecloud_pub_ =
      nh_.advertise<geometry_msgs::PoseArray>("particlecloud", 2, true);
  global_loc_srv_ = nh_.advertiseService(
      "global_localization", &AmclNode::globalLocalizationCallback, this);
  nomotion_update_srv_ = nh_.advertiseService(
      "request_nomotion_update", &AmclNode::nomotionUpdateCallback, this);
  set_map_srv_ =
      nh_.advertiseService("set_map", &AmclNode::setMapCallback, this);
  ...
  laser_scan_filter_->registerCallback(
      boost::bind(&AmclNode::laserReceived, this, _1));
  initial_pose_sub_ =
      nh_.subscribe("initialpose", 2, &AmclNode::initialPoseReceived, this);
  ...
  check_laser_timer_ =
      nh_.createTimer(laser_check_interval_,
                      boost::bind(&AmclNode::checkLaserReceived, this, _1));

  diagnosic_updater_.setHardwareID("None");
  diagnosic_updater_.add("Standard deviation", this,
...
void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level) {
  ...
  if (config.min_particles > config.max_particles) {
    ROS_WARN("You've set min_particles to be greater than max particles, this "
             "isn't allowed so they'll be set to be equal.");
  ...
  pf_init_pose_mean.v[2] =
      tf2::getYaw(last_published_pose.pose.pose.orientation);
...
void AmclNode::runFromBag(const std::string &in_bag_fn) {
  ...
  std::string scan_topic_name =
      "base_scan"; // TODO determine what topic this actually is from ROS
  topics.push_back(scan_topic_name);
  rosbag::View view(bag, rosbag::TopicQuery(topics));

  ros::Publisher laser_pub =
      nh_.advertise<sensor_msgs::LaserScan>(scan_topic_name, 100);
  ...
  ROS_INFO("Bag complete, took %.1f seconds to process, shutting down",
           runtime);
  ...
  ROS_INFO("Final location %.3f, %.3f, %.3f with stamp=%f",
           ...
           last_published_pose.pose.pose.position.y, yaw,
           last_published_pose.header.stamp.toSec());
...
void AmclNode::checkLaserReceived(const ros::TimerEvent &event) {
  ...
  if (d > laser_check_interval_) {
    ROS_WARN(
        "[AmclNode] : No laser scan received (and thus no pose updates have "
        "been published) "
...
void AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr &laser_scan) {
  ...
  if (frame_to_laser_.find(laser_scan_frame_id) == frame_to_laser_.end()) {
    ...
    try {
      ...
      ROS_ERROR("Couldn't transform from %s to %s, "
                ...
                laser_scan_frame_id.c_str(), base_frame_id_.c_str());
      return;
    }
  ...
  if (lasers_update_[laser_index]) {
    ...
    ROS_DEBUG("Laser %d angles in base frame: min: %.3f inc: %.3f", laser_index,
    ...
    if (max_weight > 0.0) {
      ...
      try {
        ...
        tf2::Transform tmp_tf(
            q, tf2::Vector3(hyps[max_weight_hyp].pf_pose_mean.v[0],
      ...
      if (tf_broadcast_ == true) {
        ...
        ros::Time transform_expiration =
            (laser_scan->header.stamp + transform_tolerance_);
...
void AmclNode::standardDeviationDiagnostics(
    diagnostic_updater::DiagnosticStatusWrapper &diagnostic_status) {
  ...
  if (std_x > std_warn_level_x_ || std_y > std_warn_level_y_ ||
      std_yaw > std_warn_level_yaw_) {
    diagnostic_status.summary(diagnostic_msgs::DiagnosticStatus::WARN,
                              "Too large");
  } else {
